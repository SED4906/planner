...
pub fn map_to(pagemap: usize, vpage: usize, pframe: usize, flags: PageFlags) -> Result<(), usize> {
    ...
    if flags.huge_page {
        unsafe {
            *(pml2 as *mut usize).add(entry_level2) =
                pframe | page_flags(flags) | if entry_level4 >= 256 { 0xFFFF << 48 } else { 0 };
        }
        return Ok(());
    }
    let pml1 = map_to_step(pml2 & !0xFFF, entry_level2)?;
    unsafe {
        *(pml1 as *mut usize).add(entry_level1) = pframe | page_flags(flags);
    }
